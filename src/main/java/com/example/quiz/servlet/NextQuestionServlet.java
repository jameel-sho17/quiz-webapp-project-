...
@WebServlet(name = "NextQuestionServlet", urlPatterns = {"/admin/session/next"})
public class NextQuestionServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            ...
            int nextIndex = s.getCurrentQuestionIndex() + 1;
            if (nextIndex >= total) {
                s.setStatus(QuizSession.Status.FINISHED);
                ...
                .setFirstResult(nextIndex)
                .setMaxResults(1)
                .getSingleResult();

                int safeDur = (dur == null) ? 20 : Math.max(1, Math.min(dur, 60));

                s.setCurrentQuestionIndex(nextIndex);
